...
use App\Support\CuentaCorriente\CuentaCorrienteMovimientosClientesTcpdf;
...
class CuentaCorrienteMovimientosClientesPdfController extends Controller
{
    public function __invoke(Request $request)
    {
        abort_unless(tienePermiso(PermisosIaCatalog::FACTURACION), 403);

        $key = 'cc-mov:clientes-pdf:'.(auth()->id() ?? 'guest');
        ...
        $clientes = CuentaCorrienteMovimientosConsulta::clientesListado($busqueda);
        ...
        $pdf = CuentaCorrienteMovimientosClientesTcpdf::generar([
            'header' => LabEntornoPdf::datosHeader(),
            'filas' => $filas,
            'saldo_total' => $saldoTotal,
        ]);

        return CuentaCorrienteMovimientosClientesTcpdf::respuestaHttp($pdf, 'cuenta-corriente-clientes.pdf');
    }
}
