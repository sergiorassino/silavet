...
class CuentaCorrienteMovimientosDetalleExcelController extends Controller
{
    public function __invoke(Request $request, int $id, CuentaCorrienteMovimientosExporter $exporter): StreamedResponse
    {
        abort_unless(tienePermiso(PermisosIaCatalog::FACTURACION), 403);

        $key = 'cc-mov:detalle-xlsx:'.(auth()->id() ?? 'guest');
