<div class="vl-page">
    ...
    <form wire:submit.prevent="save" class="vl-card mx-auto w-full max-w-md p-4">
        <div class="grid gap-3">
            <div>
                <label class="form-label mb-1" for="idEspecies">Especie *</label>
                <select wire:model="idEspecies" id="idEspecies" class="form-input py-1.5 text-sm" autofocus>
            ...
            <div>
                ...
                <input wire:model="nombre" id="nombre" type="text" maxlength="150" class="form-input py-1.5 text-sm">
                @error('nombre') <p class="form-error">{{ $message }}</p> @enderror
            </div>

            <div class="flex flex-wrap gap-2 pt-1">
                <button type="submit" class="btn-primary py-1.5 text-sm" wire:loading.attr="disabled">Guardar</button>
                <a href="{{ route('abm.razas.index') }}" class="btn-secondary py-1.5 text-sm">Cancelar</a>
            </div>
        </div>
    </form>
</div>
