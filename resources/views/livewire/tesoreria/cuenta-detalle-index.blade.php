    ...
    <div class="vl-card mx-auto max-w-3xl overflow-hidden">
        <div class="vl-toolbar border-b border-accent-200 px-3 py-2">
            <input wire:model.live.debounce.300ms="busqueda"
                   type="search"
                   placeholder="Buscar por cuenta o subcuenta…"
                   class="form-input max-w-xs py-1.5 text-sm">
        </div>

        <div class="flex justify-center overflow-x-auto px-2 py-1">
            <table class="vl-grid-pocos-campos vl-grid-angosta-wrap w-full text-sm">
                <thead class="bg-accent-50/80">
                    <tr>
                        <th class="px-3 py-2 text-left font-semibold text-neutral-700">Cuenta</th>
                        <th class="px-3 py-2 text-left font-semibold text-neutral-700">Subcuenta</th>
                        <th class="px-3 py-2 text-center font-semibold text-neutral-700 w-24">Acciones</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-accent-100">
                    @forelse ($detalles as $detalle)
                        <tr wire:key="cuenta-detalle-{{ $detalle->id }}">
                            <td class="px-3 py-2">{{ $detalle->cuenta?->nombreCuenta ?: '—' }}</td>
                            <td class="px-3 py-2 font-medium">{{ $detalle->nombreCuentasDetalle }}</td>
                            <td class="px-3 py-2">
                                <div class="flex items-center justify-center gap-0.5">
                                    <a href="{{ route('tesoreria.cuentas-detalle.edit', $detalle->id) }}"
                                    ...
                                    <x-vl-grid-icon-btn title="Eliminar subcuenta"
                                                        variant="danger"
                                                        wire:loading.attr="disabled"
                                                        x-on:click="window.vlSwalConfirmar('¿Eliminar esta subcuenta? Esta acción no se puede deshacer.', 'Eliminar subcuenta', { confirmButtonText: 'Sí, eliminar', icon: 'warning' }).then(ok => ok && $wire.eliminar({{ $detalle->id }}))">
                                        <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.75"
                    ...
                    @empty
                        <tr>
                            <td colspan="3" class="px-3 py-4 text-center text-neutral-500">
        ...
        @if ($detalles->hasPages())
            <div class="vl-matriz-list-footer px-3 py-1.5 sm:px-4">
                {{ $detalles->links('vendor.pagination.vl-compact') }}
            </div>
        @endif
    </div>
</div>
