<div class="vl-page">
    <div class="vl-hero mb-4">
        <div class="vl-hero-inner flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <x-vl-hero-heading>
                ...
                <h1 class="text-2xl font-bold sm:text-3xl">Gestión de Conceptos</h1>
                <p class="mt-2 text-sm text-white/80">Conceptos de ingreso y egreso para movimientos de caja.</p>
            </x-vl-hero-heading>
            <a href="{{ route('tesoreria.conceptos.create') }}" class="btn-primary shrink-0 bg-white text-primary-700 hover:bg-accent-50">Nuevo concepto</a>
        </div>
    </div>
    ...
    <div class="vl-card mx-auto max-w-3xl overflow-hidden">
        <div class="vl-toolbar flex flex-wrap items-center gap-2 border-b border-accent-200 px-3 py-2">
            ...
            <select wire:model.live="filtroTipo" class="form-input max-w-[12rem] py-1.5 text-sm">
        ...
        <div class="flex justify-center overflow-x-auto px-2 py-1">
            <table class="vl-grid-pocos-campos vl-grid-angosta-wrap text-sm">
                <thead class="bg-accent-50/80">
                    <tr>
                        <th class="px-3 py-2 text-left font-semibold text-neutral-700">
                            <button type="button"
                        ...
                        <th class="px-3 py-2 text-center font-semibold text-neutral-700 w-20">
                            <button type="button"
                                    wire:click="ordenar('orden')"
                                    class="inline-flex items-center justify-center gap-1 hover:text-primary-700"
                ...
                <tbody class="divide-y divide-accent-100">
                    @forelse ($conceptos as $concepto)
                        <tr wire:key="concepto-{{ $concepto->id }}">
                            <td class="px-3 py-2">{{ $concepto->tipoMovimiento?->tipoMovimiento ?? '—' }}</td>
                            <td class="px-3 py-2 font-medium">{{ $concepto->concepto }}</td>
                            <td class="px-3 py-2 text-center tabular-nums">{{ $concepto->orden ?? 0 }}</td>
                            <td class="px-3 py-2">
                                <div class="flex items-center justify-center gap-0.5">
                                    <a href="{{ route('tesoreria.conceptos.edit', $concepto->id) }}"
                                    ...
                                    <x-vl-grid-icon-btn title="Eliminar concepto"
                                                        variant="danger"
                                                        wire:loading.attr="disabled"
                                                        x-on:click="window.vlSwalConfirmar('¿Eliminar este concepto? Esta acción no se puede deshacer.', 'Eliminar concepto', { confirmButtonText: 'Sí, eliminar', icon: 'warning' }).then(ok => ok && $wire.eliminar({{ $concepto->id }}))">
                                        <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.75"
        ...
        @if ($conceptos->hasPages())
            <div class="vl-matriz-list-footer px-3 py-1.5 sm:px-4">
                {{ $conceptos->links('vendor.pagination.vl-compact') }}
            </div>
        @endif
    </div>
</div>
