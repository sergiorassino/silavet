...
| `cuit`, `PtoVta`, `key`, `crt`, `crtVencimiento` | Datos AFIP del emisor. `key`/`crt` = nombre de archivo en `afipSE/cert/{idUsuarios}/` (upload desde ABM usuarios). `crtVencimiento` = fecha de vencimiento del `.crt`, leída al cargarlo |
