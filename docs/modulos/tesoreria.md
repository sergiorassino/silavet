...
- Config: `tenant.facturacion_afip` (`modo`: `paciente` | `movimiento` | `movimiento_caja`). labvetciudad: `simular => false` (en el servidor llama a WSAA/WSFE; hace falta certificado en `afipSE/cert/{idUsuarios}/`, cargado desde Gestión de Usuarios). En local sigue simulando vía `simular_local`. El resto de tenants mantiene CAE simulado.
